...
int bruteForce_oneway(char *hash)
{
    ...
    printf("\nKnown Message: '%s'", msg_one);
    printf("\nKnown Hash: ");
    for(int i = 0; i < EVP_MAX_MD_SIZE; i++)
    {
        printf("%x", hash_one[i]);
    }
    printf("\nMatching Message: '%s'", msg_two);
    printf("\nHash of mathcing message: ");
    for(int i = 0; i < EVP_MAX_MD_SIZE; i++)
    {
        printf("%x", hash_two[i]);
    }
    // print out the number of tries it took to crack the one-way property
    printf("\nOne-way Property cracked after %d tries!\n", tries);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 5; i++)
    {
        tries += bruteForce_oneway(hash);
    }
    printf("Average tries to crack One-Way property: %d\n", tries / 5);
}